...
class PatientBill
{
    ...
    public static function add_to_each_drug_bill(EachDrug $drug, $quantity, $patient, $overwrite = false)
    {
        $items = static::get_bill();
        $exist = FALSE;
        ...
        if (!$exist) {
         ...
         //   $t->price              = $investigation->price;
            $investigation         = ProductBatch::find_product_expiring($drug->product_id);
            $prod                  = Product::find_by_id($drug->product_id);
            $t->name               = $prod->name;
    ...
    public static function decrease_bill_unit(Product $product, $quantity)
    {
        $items = static::get_bill();
        foreach ($items as $item) {
    ...
    public static function decrease_each_bill_unit(EachDrug $drug, $quantity)
    {
        $items = static::get_bill();
        foreach ($items as $item) {
    ...
    public static function save_page()
    {
        ...
        $items = static::get_bill();
        if (count($items) == 0)
            return '<h4>Your Drug Selection is empty! </h4>';
  ...
  //  public static function add_to_bill(Product $product, $quantity, $patient, $overwrite = false)
    public static function add_to_dispense(Dispensed $dispense, $unit, $patient, $overwrite = false)
    {
        $items = static::get_bill();
        $exist = FALSE;
    ...
    public static function decrease_dispense_unit(Dispensed $dispense, $unit)
    {
        $items = static::get_bill();
        foreach ($items as $item) {
...
/*    protected static function get_single_dispense_item($dispense)
    ...
    public static function drug_page()
    {
      ...
      //  print_r($items);
        ...
        if (count($items) == 0)
            return '<h4>No Drug(s) Selected!</h4>';
