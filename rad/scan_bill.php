...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $test    = EachScan::find_by_id($_GET['id']);
 //   $test    = Test::find_by_id($_GET['id']);
    ...
    if (!$test)
        ...
        $count = $_GET['action'] == 'delete' ?
            ...
            TestBill::decrease_each_scan_bill_unit($test, $unit) :
            $_GET['action'] == 'put' ?
                TestBill::decrease_each_scan_bill_unit($test, $unit) :
                TestBill::add_to_each_scan_bill($test, $unit, $patient, $overwrite);
              //  TestBill::add_to_bill($test, $unit, $patient, $overwrite);

        $result->message = "Successfully delete the test from the bill";
        $result->success = true;
        $result->bill =  TestBill::lab_page();  //  TestBill::render_bill();
